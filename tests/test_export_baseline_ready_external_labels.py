...
class BaselineReadyExportTests(unittest.TestCase):
    def test_exports_only_guarded_main_cycle_labels_and_non_leakage_features(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            target_rows = pd.read_csv(output_root / "baseline_ready_targets.csv")
            ...
            self.assertTrue(target_rows["target_threshold_crossed"].any())
            self.assertEqual(int(target_rows["target_threshold_crossed"].sum()), 1)
            self.assertNotIn("target_threshold_crossed", feature_rows.columns)
            self.assertNotIn("cycles_to_eol_at_row", feature_rows.columns)
            self.assertNotIn("capacity_delta_ah", feature_rows.columns)
            self.assertNotIn("capacity_ah_mean", feature_rows.columns)
            self.assertNotIn("sample_rows", feature_rows.columns)
            ...
            self.assertIn("sample_rows", removed["column"].tolist())
