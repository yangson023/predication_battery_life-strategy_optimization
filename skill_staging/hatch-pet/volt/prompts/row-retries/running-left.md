Create Codex pet row `running-left` for `volt`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact whole-body mascot: a friendly lithium-metal coin-cell/battery character with tiny analyst goggles, a small non-readable strategy slate built into its side, silver graphite and teal accents, clean science-lab toy/sticker style, calm focused personality for battery life prediction and optimization. No readable text, no logos, no UI screenshots.. Preserve silhouette, face, palette, material, proportions, markings, and props.
